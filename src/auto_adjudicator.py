...
def init_engine (eng_name, eng_hash=False):
    '''Engine start-up'''
...
def main():
    ...
    engine = init_engine("engine_name", 1024)
    ...
    while next_game:
        ...
        if not ((running_board.is_checkmate()) or 
                (running_board.is_stalemate())):
            ...
            pv_score = get_score(engine_data.info["score"][1], 
                    running_board.turn)
            end_node.parent.add_variation(end_node.move, starting_comment 
                    = (engine.name + " " + str(engine_data.info["depth"]) 
                    + ": " + str(pv_score)))
            end_node.parent.variation(1).add_variation (engine_data.info
                    ["pv"][1][0])
            #end_node.add_variation (, starting_comment = (engine.name +
            
            if ((pv_score >= white_threshold) and (next_game.headers["Result"] 
                    != '1-0')):
                
                next_game.headers["Result"] = "1-0"
                end_node.comment = end_node.comment + " - auto-adjusted to 1-0"
                
            elif ((pv_score <= black_threshold) and (next_game.headers
                    ["Result"] != '0-1')):
                
                next_game.headers["Result"] = "0-1"
                end_node.comment = end_node.comment + " - auto-adjusted to 0-1"
                
            elif ((pv_score < white_threshold) and (pv_score > black_threshold) 
                    and (next_game.headers["Result"] != "1/2-1/2")):
                
                next_game.headers["Result"] = "1/2-1/2"
                end_node.comment = (end_node.comment + 
                        " - auto-adjusted to 1/2-1/2")
                
            else:
                
                end_node.comment = end_node.comment + " - auto-adjudicator "
                        + "detects no result change necessary"
                
        with open (pgn_out, 'a') as f:
            f.write (str(next_game))
            f.write ("\n\n")
